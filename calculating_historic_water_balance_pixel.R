...
calc_wat_bal <- function(continent = "Europa", ncores = 15){
  ...
  watbalList <- parallel::mclapply(1:length(pixelList), FUN = function(j){
    ...
    if(!file.exists(paste0(root,"/CWR_pre-breeding/Input_data/_soils/Water_balance/",continent,"/cellID_",pixelList[j],".rds"))){
      ...
      saveRDS(object = general_watbal, file = paste0(root, "/CWR_pre-breeding/Input_data/_soils/Water_balance/",continent,"/cellID_",pixelList[j],".rds"))
